...
export class Simulation {
  constructor(options = {}) {
    ...
    //
    // First time the loss drops below convergenceLossThreshold (i.e. the
    // crossing event, regardless of later bouncing back during EoS
    // oscillations), record the step. Stop training convergenceCooldownSteps
    // steps after that — giving the run some time to settle once it's
    // "essentially learned the task." The user can hit start again to resume.
    //
    // Set convergenceLossThreshold to 0 (or negative) to disable.
    ...
    this.convergenceCooldownSteps = options.convergenceCooldownSteps !== undefined
      ...
      : 200;
    this.convergenceCrossStep = null;   // step at which loss first dipped below threshold
    this.autoStopFired = false;         // set once cooldown elapsed; cleared on reset
  ...
  runLoop() {
    ...
    while (true) {
      ...
      // Auto-stop: record the first step at which loss drops below
      // convergenceLossThreshold; once cooldown elapses, stop (once per
      // run — re-pressing start lets the user keep going past auto-stop).
      if (
        this.convergenceLossThreshold > 0 &&
        !this.autoStopFired &&
        this.convergenceCrossStep === null &&
        loss < this.convergenceLossThreshold
      ) {
      ...
      if (
        !this.autoStopFired &&
        this.convergenceCrossStep !== null &&
        this.iteration >= this.convergenceCrossStep + this.convergenceCooldownSteps
      ) {
