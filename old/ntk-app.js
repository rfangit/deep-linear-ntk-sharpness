...
function renderNTKGrid(container, blocks, inputLabels) {
  ...
  inputLabels.forEach(lbl => {
    ...
    h.style.cssText = `width:${n * 84}px; text-align:center; font-size:12px; color:#888; font-family:Georgia,serif;`;
    h.innerHTML = `$x' = ${lbl}$`;
  ...
  inputLabels.forEach((rowLbl, ri) => {
    ...
    rl.style.cssText = 'width:70px; text-align:right; padding-right:10px; font-size:12px; color:#888; font-family:Georgia,serif;';
    ...
    inputLabels.forEach((colLbl, ci) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < n; j++) {
          ...
          td.style.cssText = `
            width:80px; height:52px; text-align:center; vertical-align:middle;
...
function drawBars(canvas, values, opts = {}) {
  ...
  if (opts.title) {
    ...
    ctx.fillText(opts.title, pad.l + 4, pad.t + 12);
  }
...
function initW1() {
  ...
  const genBtn = document.getElementById('w1-generate');
...
function initW4() {
  ...
  function render() {
    ...
    drawBars(eigCanvas, orderedEigVals,
      {title: 'NTK eigenvalues λα', yMax: fixedEigYMax});
    drawBars(residCanvas, residProj,
      {title: '|r · eα|  (residual projection)', yMax: fixedResidYMax});
  }
